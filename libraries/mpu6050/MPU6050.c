...
struct mpu6050 mpu6050_init(i2c_inst_t *i2c_instance, const uint8_t address)
{
    ...
    mpu6050.config.use_threshold = 0;
...
void mpu6050_calibrate_gyro(struct mpu6050 *self, uint32_t samples)
{
...
void mpu6050_find_offset_values(struct mpu6050 *self, uint16_t n) {
    
    const float proportional_gain = 0.1f;
    const float integral_gain = 0.03f;

    float accel_integral_x = 0;
    float accel_integral_y = 0;
    float accel_integral_z = 0;

    float gyro_integral_x = 0;
    float gyro_integral_y = 0;
    float gyro_integral_z = 0;

    const float accel_expected_x = 0;
    const float accel_expected_y = 0;
    const float accel_expected_z = 9.81f;

    const float gyro_expected_x = 0;
    const float gyro_expected_y = 0;
    const float gyro_expected_z = 0;
    ...
    for(uint16_t i = 0; i < n; i++) {
        ...
        mpu6050_vectorf_t * accel = mpu6050_get_accelerometer(self);
        mpu6050_vectorf_t * gyro = mpu6050_get_gyroscope(self);

        accel->x += accel_offset_X;
        accel->y += accel_offset_y;
        accel->z += accel_offset_z;

        gyro->x += gyro_offset_x;
        gyro->y += gyro_offset_y;
        gyro->z += gyro_offset_z;
        
        // Calculate Error
        float accel_error_x = accel_expected_x - accel->x;
        float accel_error_y = accel_expected_y - accel->y;
        float accel_error_z = accel_expected_z - accel->z;

        float gyro_error_x = gyro_expected_x - gyro->x;
        float gyro_error_y = gyro_expected_y - gyro->y;
        float gyro_error_z = gyro_expected_z - gyro->z;

        // Calculate integral
        accel_integral_x += accel_error_x * integral_gain;
        accel_integral_y += accel_error_y * integral_gain;
        accel_integral_z += accel_error_z * integral_gain;

        gyro_integral_x += gyro_error_x * integral_gain;
        gyro_integral_y += gyro_error_y * integral_gain;
        gyro_integral_z += gyro_error_z * integral_gain;
        
        if(accel_integral_x > 0.3) accel_integral_x = 0.3;
        if(accel_integral_x < -0.3) accel_integral_x = -0.3;
        if(accel_integral_y > 0.3) accel_integral_y = 0.3;
        if(accel_integral_y < -0.3) accel_integral_y = -0.3;
        if(accel_integral_z > 0.3) accel_integral_z = 0.3;
        if(accel_integral_z < -0.3) accel_integral_z = -0.3;

        if(gyro_integral_x > 0.3) gyro_integral_x = 0.3;
        if(gyro_integral_x < -0.3) gyro_integral_x = -0.3;
        if(gyro_integral_y > 0.3) gyro_integral_y = 0.3;
        if(gyro_integral_y < -0.3) gyro_integral_y = -0.3;
        if(gyro_integral_z > 0.3) gyro_integral_z = 0.3;
        if(gyro_integral_z < -0.3) gyro_integral_z = -0.3;

        // Calculate pid values
        float accel_pid_x = accel_error_x * proportional_gain + accel_integral_x;
        float accel_pid_y = accel_error_y * proportional_gain + accel_integral_y;
        float accel_pid_z = accel_error_z * proportional_gain + accel_integral_z;

        float gyro_pid_x = gyro_error_x * proportional_gain + gyro_integral_x;
        float gyro_pid_y = gyro_error_y * proportional_gain + gyro_integral_y;
        float gyro_pid_z = gyro_error_z * proportional_gain + gyro_integral_z;

        // Add pid values to the offsets
        accel_offset_X += accel_pid_x;
        accel_offset_y += accel_pid_y;
        accel_offset_z += accel_pid_z;

        gyro_offset_x += gyro_pid_x;
        gyro_offset_y += gyro_pid_y;
        gyro_offset_z += gyro_pid_z;

    }

    self->config.use_threshold = 1;

    self->of.accel_offset_x = accel_offset_X;
    self->of.accel_offset_y = accel_offset_y;
    self->of.accel_offset_z = accel_offset_z;

    self->of.gyro_offset_x = gyro_offset_x;
    self->of.gyro_offset_y = gyro_offset_y;
    self->of.gyro_offset_z = gyro_offset_z;
    return;
}
...
{
    if(self->config.use_threshold == 1){
        ...
        {
            self->na.x = self->ra.x * self->config.range_per_digit * GRAVITY_CONSTANT + self->of.accel_offset_x;
            self->na.y = self->ra.y * self->config.range_per_digit * GRAVITY_CONSTANT + self->of.accel_offset_y;
            self->na.z = self->ra.z * self->config.range_per_digit * GRAVITY_CONSTANT + self->of.accel_offset_z;
